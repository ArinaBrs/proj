...
# Имя файла
file_name = '119-130 оборот розничной торговли непродовольственными.xls'
# Папка, в которой лежит файл
folder = r"info-stat-12-2016\05 торговля"
# Коды листов файла
...
def getFile(file, sheet_name):
    ...
    xls_file.drop(xls_file.tail(len(xls_file.index) - regions_count).index, inplace=True)
...
try:
    ...
    for i in range(len(sheets)):
        sheet_name = sheets[i]
        code = codes[i]
        file = getFile(full_file_name, sheet_name)
